...
public class DayFour {
    ...
    //Part one
    public static boolean getValidityPartOne(String input) {
    ...
    //Part two
    public static boolean getValidityPartTwo(String input) {
        ArrayList<String> phrases = new ArrayList<String>();
        for(String phrase : input.split("\\s+")) {
            phrase = Stream.of(phrase.split("")).sorted().collect(Collectors.joining());
            if(phrases.contains(phrase))
                return false;
            else
                phrases.add(phrase);
    ...
    public static void main(String[] args) {
        ...
        for(String inputLine : input.split("\n")) {
            result += DayFour.getValidityPartOne(inputLine) ? 1 : 0;
        }
        System.out.printf("Part one: %d", result);
        result = 0;
        for(String inputLine : input.split("\n")) {
            result += DayFour.getValidityPartTwo(inputLine) ? 1 : 0;
        }
        System.out.printf("%nPart two: %d", result);
    }

    private static final String input = "oaoe rxeq vssdqtu xrk cjv yaoqp loo\n" +
