...
public class DaySeven {
    public static String getBaseTower(String input) {
        Map<String, Integer> characters = new HashMap<String, Integer>();

        for(String connectionLine : input.split("\n")) {
            connectionLine = connectionLine.replace(" -> ", " ")
                                            .replaceAll(".\\(.*\\)", "")
                                            .replaceAll(", ", " ");
            for(String s : connectionLine.split(" "))
                if(!characters.containsKey(s))
                    characters.put(s, 1);
                else
                    characters.put(s, characters.get(s) + 1);
        }
        return lowestCount(characters);
    }

    private static String lowestCount(Map<String, Integer> characters) {
        int baseNumber = Integer.MAX_VALUE;
        String baseCharacter = "";
        for(String key : characters.keySet()) {
            if(characters.get(key) < baseNumber) {
                baseNumber = characters.get(key);
                baseCharacter = key;
            }
        }
    ...
    public static void main(String[] args) {
        System.out.printf("Part one: %s", DaySeven.getBaseTower(input));
    }

    private static final String input = "jlbcwrl (93)\n" +
